...
class PhysNetEnergy(nn.Module):
    ...
    def __init__(self, cutoff: float = 10.0):
        ...
        self.cutoff_fn = PhysNetCutOff(cutoff)
        
        
#         self.register_parameter('key', nn.Parameter(torch.Tensor(n_atom_basis)))
#         self.register_parameter('qfeatures', nn.Parameter(torch.Tensor(self.num_features)))
#         nn.init.zeros_(self.key)
