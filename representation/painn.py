...
from schnetpack.representation.support import Residual, Output, Module, PhysNetCutOff
...
class PaiNNInteraction(nn.Module):
    ...
    def forward(
        ...
        ## intra-atomic
        mu_mix = self.mu_channel_mix(mu.float())
...
class PaiNN(nn.Module):
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]):
        ...
        d_ij = torch.norm(r_ij, dim=1, keepdim=True).float()
        dir_ij = r_ij / d_ij
        ...
        for i, interaction in enumerate(self.interactions):
            q, mu = interaction(q.float(), mu.float(), filter_list[i], dir_ij, idx_i, idx_j, n_atoms)
